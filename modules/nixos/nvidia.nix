{ pkgs, vars, ... }: {
  boot = {
    ...
    kernelModules = vars.boot.kernelModules
      ++ [ "bbswitch" "nvidia" "nvidia_drm" "nvidia_uvm" "nvidia_modeset" ];
  ...
  hardware = {
    ...
    opengl.extraPackages = (vars.hardware.opengl.extraPackages pkgs)
      ++ (with pkgs; [ nvidia-vaapi-driver ]);
  };
  environment.variables.NVD_BACKEND = "direct";
  services.xserver.videoDrivers = vars.services.xserver.videoDrivers
    ++ [ "nvidia" ];
}
