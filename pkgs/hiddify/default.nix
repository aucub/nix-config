{ lib, stdenv, appimageTools, fetchzip, }:
let
  ...
  hiddify-src = stdenv.mkDerivation {
    ...
    src = fetchzip {
      stripRoot = false;
      url =
        "https://github.com/hiddify/hiddify-next/releases/download/v${version}/hiddify-linux-x64.zip";
...
in appimageTools.wrapType2 rec {
  ...
  meta = with lib; {
    description =
      "A multi-platform client based on Sing-box that serves as a universal proxy tool-chain.";
